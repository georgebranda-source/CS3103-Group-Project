...
/*
 * Purpose: Entry point of the program. Parses the specified text file to extract configuration, initializes shared data structures, spawns threads, waits for completion, and cleans up resources.
 * Args: argc - Number of command-line arguments.
 * argv - Array of command-line argument strings (expects the input file path at argv[1]).
 * Return: 0 on successful execution, 1 on file or argument error.
 */
int main(int argc, char* argv[]) {
    ...
    // Open the text file containing the configuration
    ...
    if (fscanf(fp, "%d", &P) != 1) return 1;
    if (fscanf(fp, "%d", &M) != 1) return 1;
    if (fscanf(fp, "%d", &N) != 1) return 1;
    if (fscanf(fp, "%d", &num_orders) != 1) return 1;
    ...
    // Read line 6: Token counts
    for (int i = 0; i < T; i++) {
        if (fscanf(fp, "%d", &token_pool.available_tokens[i]) != 1) return 1;
    }

    // Read line 7: Token pairs (e.g., "(0,1) (1,2)")
    for (int i = 0; i < P; i++) {
        // The leading space in the format string tells fscanf to skip whitespace/newlines
        if (fscanf(fp, " (%d,%d)", &encoder_reqs[i].tA, &encoder_reqs[i].tB) != 2) return 1;
    }
